...
| 2026-08-04 | E2E: intent SDK → pinpad con variantes qa/dev | ⚠️ El SDK apunta al package de release (`com.payclip.blaze.pinpad`); con pinpad QA instalado (`.qa`) → `APPLICATION_NOT_FOUND` (entregado por listener, sin crash — fail-open OK). Para pruebas se apuntó temporalmente a `.qa` en `SystemClipIntentProvider` (**no commitear**). Pendiente: mecanismo de override del package destino para QA/dev |
