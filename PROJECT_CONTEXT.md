...
> **🔄 PIVOTE AL APPROACH 2 (2026-08-05) — ESTE ES EL APPROACH VIGENTE.** Sesión forzada por
> credenciales del SDK: en vez de bloquear el cobro cuando la sesión de pinpad no corresponde,
> se reemplaza por la cuenta de las credenciales de `setLoginCredentials` (fail-closed si el
> login falla: `SESSION_REPLACEMENT_FAILED`, jamás se cobra con la cuenta anterior; resultado
> con `wasSessionReplaced`). **Doc de diseño, decisiones, matriz E2E y pendientes:
> `VALIDACION_SESION_SDK.md` en el repo pinpad** (rama
> `adanantonio/f2f-834-approach2-sesion-forzada`) — leer ese doc antes que las secciones de
> approach 1 de este archivo, que quedan como referencia histórica.
>
...
| 2026-08-05 | **Pivote al approach 2 (sesión forzada)** e implementación en ambos repos, en ramas nuevas basadas en main (el approach 1 queda pausado en sus ramas). SDK: `wasSessionReplaced` en `PaymentResult`, overload default `PaymentListener.onFailure(code, result)`, `-Xjvm-default=all`, consumer-rules, fix del tip, README. Pinpad: `EvaluateSdkSessionUseCase`, `resolveSdkSession` (login-primero, fail-closed), `SdkSessionReplacementHolder`, `SdkSessionTracker` | Ambos repos, ramas `*-approach2-*` | F2F-834 |
| 2026-08-12 | **Commits en ambos repos** (antes vivía en working tree/stash): SDK `1f74b21` (feature) + `b18bcb5` (WIP package `.qa` — revertir antes del PR); pinpad re-basado en main `fab77b7`: `ea72a6e` (feature), `3e3640e` (logs `SessionValidation`), `e4bfca0` (WIP plomería — revertir), `2320464` (bitácora con matriz E2E). **E2E en L200/stage: 7/8 ✅** (falta regresión SDK 1.1.0). SDK `1.2.0-local` publicado a mavenLocal para las pruebas | Ambos repos | F2F-834 |
| 2026-08-13 | **Review de Copilot atendido en pinpad #287** (sin comentarios en el PR #47 de este repo): logs `SessionValidation` eliminados por completo, `trim()` de credenciales retirado, tests de `EvaluateSdkSessionUseCase` (7/7). Queda abierto detener `OrderListenerService` en reemplazo exitoso (se revisa al final). **Check Dependency Review roto en este repo — causa raíz encontrada**: la org tiene **IP allow list** y este repo público cae en runners hosted de GitHub (IP no permitida → 403); los repos privados pasan porque usan el runner group de la org. Ni v1 ni v4 pueden pasar aquí sin acción de DevOps. PR [#48](https://github.com/ClipMX/mobile.android.blaze.pinpad.sdk/pull/48) actualiza el workflow a v4 (patrón de la org) y documenta el diagnóstico y las opciones en un comentario. El check NO es requerido: no bloquea merges | `.github/workflows/dependecy-review.yml` (PR #48, rama `adanantonio/update-dependency-review-v4`) | F2F-834 |
...
| 2026-08-05 | **Pivote al approach 2: sesión forzada** en vez de request + ownership. Corregir la sesión (la venta siempre cae en la cuenta de las credenciales del SDK) en lugar de detectar y bloquear | Cero dependencia de BE, cero latencia en happy path, subcuentas resueltas por diseño; el logout forzado de la cuenta discrepante es feature, no efecto colateral. Decisiones detalladas en `VALIDACION_SESION_SDK.md` (repo pinpad) |
...
- [ ] Actualizar tickets de Linear al approach 2 y decidir el destino de las subtareas del approach 1.
- [ ] Tests unitarios.

Detalle completo en `VALIDACION_SESION_SDK.md` (repo pinpad). Las secciones siguientes son del
**approach 1 (pausado)** y se conservan como referencia.

### Sesión 2026-08-05 (agenda acordada — approach 1, pausado)
